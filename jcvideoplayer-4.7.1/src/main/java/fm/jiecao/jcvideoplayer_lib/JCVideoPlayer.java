...
//                        mDownY = y;
    ...
    public void onPrepared() {
        Log.i(TAG, "准备播放：onPrepared " + " [" + this.hashCode() + "] ");
        ...
        if (seekToInAdvance != 0) {
            ...
            Log.i(TAG, "onPrepared: 设置进度-----" + position);
            if (position != 0) {
...
//                .findViewById(Window.ID_ANDROID_CONTENT);
    ...
    public void onCompletion() {
        ...
        if (currentState == CURRENT_STATE_PLAYING || currentState == CURRENT_STATE_PAUSE) {
            ...
            Log.i(TAG, "onCompletion: 保存进度值：" + position);
//            int duration = getDuration();
...
//            CLICK_QUIT_FULLSCREEN_TIME = System.currentTimeMillis();
    ...
    private boolean SEEK_TOUCH = false;
    ...
    public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
        ...
        if (SEEK_TOUCH) {
            int mSeekTimePosition = progress * getDuration() / 100;
            int totalTimeDuration = getDuration();
            String seekTime = JCUtils.stringForTime(mSeekTimePosition);
            String totalTime = JCUtils.stringForTime(totalTimeDuration);
            if (mSeekTimePosition > totalTimeDuration) {
            ...
            showProgressDialog(progress, seekTime, mSeekTimePosition, totalTime, totalTimeDuration);
        }
